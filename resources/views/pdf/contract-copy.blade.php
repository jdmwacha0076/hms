...
<body>
    ...
    <div class="addresses">
        ...
        <div class="receiver-address">
            <p>Ofisi ya Serikali za Mitaa<br>Mtaa wa {{ $street_name }}<br>{{ \Carbon\Carbon::now()->format('d-m-Y') }}</p>
        </div>
    </div>
    ...
    <div class="contract-terms">
        <p>Ninakuandikia kukutambulisha ya kuwa Ndugu, <strong>{{ $tenant_name }}</strong> mwenye kitambulisho cha <strong>{{ $id_type }}</strong> chenye namba <strong>{{ $id_number }}</strong>, kuwa ni mpangaji katika nyumba yangu iliyopo mtaa wa <strong>{{ $street_name }}</strong> katika kiwanja namba <strong>{{ $plot_number }}</strong>. Mkataba wake umeanza rasmi tarehe <strong>{{ $start_date }}</strong> na unatarajiwa kumalizika tarehe <strong>{{ $end_date }}</strong>.</p>
